...
async function seed() {
  ...
  for (const item of data) {
    ...
    const payload = {
      ...
      salesUnit: item.unit,
      conversionFactor: 1,
      sellingPriceRupees: item.rate,
      purchaseCostRupees: item.rate,
      gstRate: 0,
      reorderLevel: 0,
      reorderQuantity: 10
    };
    try {
      const response = await fetch('https://mudit-traders-fc35.vercel.app/api/products', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        console.error(`Failed to add ${item.productName}:`, errText);
      } else {
        successCount++;
      }
    } catch (e) {
      console.error(`Fetch error for ${item.productName}:`, e.message);
    }
  }
  console.log(`Successfully imported ${successCount} products via API.`);
}
